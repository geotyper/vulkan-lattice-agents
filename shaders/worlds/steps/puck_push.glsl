...
void puckPushScenarioAfterStep(inout Agent agent) {
    ...
    const vec2 heading = vec2(cos(agent.pose.z), sin(agent.pose.z));
    const float contribution = puckPushContribution(agent.pose.xy, heading, agent.internal.x,
                                                    agent.pose.w, agent.penalties.yz, radius);
