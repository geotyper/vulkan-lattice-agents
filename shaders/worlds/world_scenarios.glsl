...
#include "worlds/gate_plate.glsl"
...
vec2 scenarioBeaconPosition(uint scenario, Agent agent, uint beaconIndex, uint phase,
    ...
    if (scenario == 10u) {
        return gatePlateScenarioPosition(beaconIndex, worldRadius);
    }
...
vec3 scenarioBeaconColor(uint scenario, uint beaconIndex, uint phase, uint trial) {
    ...
    if (scenario == 10u) {
        return gatePlateScenarioColor(beaconIndex);
    }
...
vec2 scenarioObstacleCentre(uint scenario, Agent agent, uint index, float worldRadius,
    ...
    if (scenario == 10u) {
        return gateBoxCentre(index, worldRadius, gatePlateScenarioOpen(agent));
    }
    return vec2(0.0);
}
...
vec2 scenarioObstacleHalfExtent(uint scenario, uint index, float worldRadius) {
    ...
    if (scenario == 10u) {
        return gateBoxHalfExtent(index, worldRadius);
    }
    return vec2(0.0);
}
...
vec3 scenarioBodyTint(uint scenario, Agent agent, vec3 bodyColor) {
    ...
    if (scenario == 10u) {
        return gatePlateScenarioBodyTint(agent, bodyColor);
    }
    return bodyColor;
}
